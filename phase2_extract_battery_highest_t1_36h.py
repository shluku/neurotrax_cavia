...
TZ = "Asia/Jerusalem"
OUT_DIR = Path("output/analysis_candidates/phase2_feature_extraction/battery_highest_t1_36h")
...
def is_charging_or_plugged(obj: dict[str, Any]) -> bool | None:
    ...
    if adaptor is None and status is None:
        return None
    adaptor_charging = adaptor is not None and adaptor != 0
    status_charging = status is not None and int(status) in {2, 5}
    return adaptor_charging or status_charging
...
def compute_features(rows: list[dict[str, Any]]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        percent = battery_percent(obj) if obj else None
        charging = is_charging_or_plugged(obj) if obj else None
        ...
        expanded_rows.append(
            {
                ...
                "battery_scale": obj.get("battery_scale"),
                "battery_percent": percent,
                "battery_status": obj.get("battery_status"),
                "battery_adaptor": obj.get("battery_adaptor"),
                "charging_or_plugged": charging,
                "battery_voltage": obj.get("battery_voltage"),
                "battery_temperature": obj.get("battery_temperature"),
                "battery_health": obj.get("battery_health"),
                "battery_technology": obj.get("battery_technology"),
            }
        )
    ...
    if not rows:
        return (
            {
                "low_battery_event_count": pd.NA,
                "charging_or_plugged_event_count": pd.NA,
                "battery_rows_in_window": 0,
                "usable_battery_level_rows": 0,
                "usable_charging_status_rows": 0,
                "json_parse_errors": 0,
    ...
    return (
        {
            ...
            "charging_or_plugged_event_count": charging_or_plugged_event_count,
            ...
            "feature_status": "ok" if usable_battery_level_rows or usable_charging_status_rows else "insufficient_json_fields",
        },
        expanded_rows,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract selected battery features for highest global_T1 patient Phase B 24h window.")
    ...
    try:
        ...
        if total_primary_rows > 0:
            ...
            if not first_candidates:
                ...
                n_rows, coverage_first_ts, coverage_last_ts = count_rows(
                    conn,
                    device_id=first_candidates[0][1],
                    start_ms=int(selected_start_ms),
                    end_ms=int(selected_end_ms),
                )
                coverage_rows.append(
                    {
                        "Subject_ID_D": subject_id_d,
        ...
        features, expanded_rows = compute_features(rows)
    finally:
        conn.close()
    ...
    features_path = OUT_DIR / "battery_highest_t1_36h_features.csv"
    coverage_path = OUT_DIR / "battery_highest_t1_36h_window_coverage.csv"
    rows_path = OUT_DIR / "battery_highest_t1_36h_rows_expanded.csv"
    readme_path = OUT_DIR / "README_battery_highest_t1_36h.md"

    pd.DataFrame([feature_row]).to_csv(features_path, index=False)
    pd.DataFrame(coverage_rows).to_csv(coverage_path, index=False)
    pd.DataFrame(expanded_rows).to_csv(rows_path, index=False)
    readme_path.write_text(
        f"""# battery Highest-T1 Phase B 24h Selected Features
...
- Highest-T1 patient: Subject_ID_D `{subject_id_d}`, global_T1 `{patient["global_T1"]}`.
- Primary start: local midnight on the day after `T1_date_iso`.
- Window length: {args.hours} hours.
- Fallback: first observed `battery` timestamp that allows a complete {args.hours}-hour span inside the T1 week.
- T1 week: `{week_start.strftime("%Y-%m-%d %H:%M:%S%z")}` to `{week_end.strftime("%Y-%m-%d %H:%M:%S%z")}`.

Result:

- window_rule: `{window_rule}`
- rows in selected window: `{features["battery_rows_in_window"]}`
...
""",
    ...
    print(f"rows_in_selected_window: {features['battery_rows_in_window']}")
    print(f"low_battery_event_count: {features['low_battery_event_count']}")
    print(f"charging_or_plugged_event_count: {features['charging_or_plugged_event_count']}")
