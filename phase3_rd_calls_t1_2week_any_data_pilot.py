...
TABLE_NAME = "calls"
OUT_DIR = ROOT / "output/analysis_candidates/phase3_rd/calls_t1_2week_any_data_pilot"
LONG_PATH = OUT_DIR / "phase3_rd_calls_t1_2week_any_data_long.csv"
WIDE_PATH = OUT_DIR / "phase3_rd_calls_t1_2week_any_data_wide.csv"
STATUS_PATH = OUT_DIR / "phase3_rd_calls_t1_2week_any_data_status.csv"
README_PATH = OUT_DIR / "README_phase3_rd_calls_t1_2week_any_data.md"


def t1_2week_window(patient: pd.Series) -> dict:
    ...
    return {
        "window_rule": "rd_t1_2week_any_calls_data",
...
def build_readme() -> str:
    return """# Phase 3 R&D: Calls T1 Two-Week Any-Data Pilot
...
def main() -> None:
    ...
    try:
        for index, (_, patient) in enumerate(patients.iterrows(), start=1):
            ...
            window = t1_2week_window(patient)
            ...
            if not device_ids:
                ...
                try:
                    ...
                    if rows:
                        ...
                        table_status = "missing_no_calls_rows_in_t1_2week"
                ...
                long_rows.append(
                    {
                        ...
                        "rows_in_window": len(rows),
                        "device_ids_used": ";".join(sorted(device_ids_used)),
                        "calculation_context": "phase3_rd_calls_t1_2week_any_data",
                        "error_message": error_message,
                    }
                )
    finally:
    ...
    print(f"patients_with_calls_in_t1_2week: {calculated_patients}")
