...
TZ = "Asia/Jerusalem"
OUT_DIR = Path("output/analysis_candidates/phase2_feature_extraction/applications_foreground_highest_t1_36h")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract selected applications_foreground features for highest global_T1 patient Phase B 24h window.")
    ...
    try:
        ...
        if total_primary_rows > 0:
            ...
            if not first_candidates:
                window_rule = "no_app_data_with_24h_span_in_T1_week"
    ...
    feature_row = {
        ...
        "feature_table": "applications_foreground",
        ...
        **features,
        "data_availability_note": "features computed from observed rows only; missing data is not zero activity",
    }

    features_path = OUT_DIR / "applications_foreground_highest_t1_36h_features.csv"
    coverage_path = OUT_DIR / "applications_foreground_highest_t1_36h_window_coverage.csv"
    rows_path = OUT_DIR / "applications_foreground_highest_t1_36h_rows.csv"
    readme_path = OUT_DIR / "README_applications_foreground_highest_t1_36h.md"

    pd.DataFrame([feature_row]).to_csv(features_path, index=False)
    pd.DataFrame(coverage_rows).to_csv(coverage_path, index=False)
    row_out = []
    for row in rows:
        obj = parse_json(row.get("data")) or {}
        row_out.append(
            {
                "_id": row.get("_id"),
                ...
                "is_system_app": obj.get("is_system_app"),
            }
        )
    pd.DataFrame(row_out).to_csv(rows_path, index=False)

    readme_path.write_text(
        f"""# applications_foreground Highest T1 Phase B 24h Extraction
...
- Primary duration: {args.hours} hours.
- If the primary window has no `applications_foreground` data across mapped devices, the fallback starts at the first observed `applications_foreground` timestamp that allows a complete {args.hours}-hour span inside the T1 week.
...
""",
    ...
    print("selected_highest_t1_patient:")
    print(f"- Subject_ID_D={subject_id_d}")
    print(f"- Subject_ID_N={patient['Subject_ID_N']}")
    print(f"- global_T1={patient['global_T1']}")
    print(f"- T1_date_iso={patient['T1_date_iso']}")
    print(f"window_rule={window_rule}")
    print(f"window_start_local={selected_start_local}")
    print(f"window_end_local={selected_end_local}")
    print("features:")
    for key in ["app_foreground_event_count", "unique_foreground_apps", "app_use_diversity", "json_parse_errors"]:
