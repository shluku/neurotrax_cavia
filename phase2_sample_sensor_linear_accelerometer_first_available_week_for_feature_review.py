...
OUT_DIR = ROOT / "output/analysis_candidates/phase2_feature_review/sensor_linear_accelerometer"
SAMPLE_LIMIT = 20
MIN_ROWS = 20
WINDOW_DAYS = 7


def first_available_ts_after_t1(conn, quoted_table: str, device_id: str, t1_start_ms: int) -> int | None:
    cur = conn.cursor()
    try:
        cur.execute(
            f"""
...
def find_window(conn, quoted_table: str) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    ...
    for _, patient in ranked.iterrows():
        ...
        for device_id in device_ids:
            first_ts = first_available_ts_after_t1(conn, quoted_table, device_id, t1_start_ms)
            if first_ts is None:
...
def write_outputs(window: dict[str, Any] | None, coverage_rows: list[dict[str, Any]], rows: list[dict[str, Any]]) -> list[Path]:
    ...
    prefix = "sensor_linear_accelerometer_adjusted_first_available_7d"
    sample_path = OUT_DIR / f"{prefix}_sample_rows.csv"
    expanded_path = OUT_DIR / f"{prefix}_sample_rows_expanded.csv"
    jsonl_path = OUT_DIR / f"{prefix}_sample_rows.jsonl"
    keys_path = OUT_DIR / f"{prefix}_json_key_summary.csv"
    coverage_path = OUT_DIR / f"{prefix}_coverage_scan.csv"
    readme_path = OUT_DIR / "README_sensor_linear_accelerometer_adjusted_first_available_7d_review.md"
...
"""
        ...
        window_text = "\nNo mapped ranked patient had at least 20 rows in the first 7 days after first available data.\n"

    readme_path.write_text(
        f"""# sensor_linear_accelerometer Adjusted First-Available 7-Day Review
...
def main() -> None:
    ...
    print("scan_rule: adjusted_first_available_7d_after_T1")
