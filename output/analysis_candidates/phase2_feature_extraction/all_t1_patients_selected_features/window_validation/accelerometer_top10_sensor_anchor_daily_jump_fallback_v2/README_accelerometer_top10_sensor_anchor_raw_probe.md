...
- Patient-level table: `output/analysis_candidates/phase2_feature_extraction/all_t1_patients_selected_features/window_validation/accelerometer_top10_sensor_anchor_daily_jump_fallback_v2/accelerometer_top10_sensor_anchor_raw_probe_patient_windows.csv`
...
- Patient rows: 3
- Metadata candidates probed: 5
- Hourly probe rows: 0

Patient window status counts:

- `raw_window_found_from_sensor_anchor_probe`: 3
...
- If the short probe misses, scan the metadata window in 24-hour jumps with `LIMIT 1`; if a day hits, use that day as the candidate 24-hour window.
...
- `daily_jump_fallback_used=True` means the short probe missed and the script tried 24-hour jumps within the metadata window.
