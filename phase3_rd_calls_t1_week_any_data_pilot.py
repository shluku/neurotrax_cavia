...
TABLE_NAME = "calls"
OUT_DIR = ROOT / "output/analysis_candidates/phase3_rd/calls_t1_week_any_data_pilot"
LONG_PATH = OUT_DIR / "phase3_rd_calls_t1_week_any_data_long.csv"
WIDE_PATH = OUT_DIR / "phase3_rd_calls_t1_week_any_data_wide.csv"
STATUS_PATH = OUT_DIR / "phase3_rd_calls_t1_week_any_data_status.csv"
README_PATH = OUT_DIR / "README_phase3_rd_calls_t1_week_any_data.md"


def t1_week_window(patient: pd.Series) -> dict[str, Any]:
    t1_date = pd.Timestamp(str(patient["T1_date_iso"])).tz_localize(TZ)
    week_start = t1_date
    week_end = week_start + pd.Timedelta(days=7)
    return {
        "window_rule": "rd_t1_week_any_calls_data",
        "window_start_ms": local_to_ms(week_start),
        "window_end_ms": local_to_ms(week_end),
        "window_start_local": week_start.strftime("%Y-%m-%d %H:%M:%S%z"),
        "window_end_local": week_end.strftime("%Y-%m-%d %H:%M:%S%z"),
    }


def selected_call_features() -> pd.DataFrame:
...
def build_readme() -> str:
    return """# Phase 3 R&D: Calls T1-Week Any-Data Pilot
...
def main() -> None:
    ...
    selected = selected_call_features()
        ...
        for index, (_, patient) in enumerate(patients.iterrows(), start=1):
            ...
            if not device_ids:
                ...
                try:
                    ...
                    if rows:
                        features = compute_calls(rows)
                        table_status = "calculated"
                    else:
                        table_status = "missing_no_calls_rows_in_t1_week"
                ...
                long_rows.append(
                    {
                        ...
                        "rows_in_window": len(rows),
                        "device_ids_used": ";".join(sorted(device_ids_used)),
                        "calculation_context": "phase3_rd_calls_t1_week_any_data",
                        "error_message": error_message,
                    }
                )
    finally:
    ...
    print(f"patients_with_calls_in_t1_week: {calculated_patients}")
