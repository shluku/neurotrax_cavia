...
def get_first_last_ts_ms(
    device_id: str,
    conn,
    cur,
    retries: int,
    retry_sleep: float,
) -> Tuple[Optional[int], Optional[int], object, object]:
    def q_minmax(c):
        c.execute(
            """
    ...
    row, conn, cur = run_with_retry(q_minmax, conn, cur, retries, retry_sleep)
...
def get_device_day_buckets(
    ...
    retries: int,
    retry_sleep: float,
) -> Tuple[List[Tuple[int, Optional[int]]], object, object]:
    def q_days(c):
        if with_counts:
            c.execute(
                """
    ...
    rows, conn, cur = run_with_retry(q_days, conn, cur, retries, retry_sleep)
    buckets: List[Tuple[int, Optional[int]]] = []
    for row in rows:
        if row[0] is None:
            continue
        if with_counts:
            buckets.append((int(row[0]), int(row[1] or 0)))
        else:
            buckets.append((int(row[0]), None))
    return buckets, conn, cur


def ensure_device_ts_index(conn, cur, retries: int, retry_sleep: float) -> Tuple[object, object]:
    def q_check(c):
        c.execute(
            """
            SHOW INDEX FROM accelerometer
            WHERE Key_name = 'idx_acc_device_ts'
            """
        )
        return c.fetchall()

    rows, conn, cur = run_with_retry(q_check, conn, cur, retries, retry_sleep)
    if rows:
        return conn, cur
    ...
    def q_create(c):
        c.execute(
            """
            CREATE INDEX idx_acc_device_ts ON accelerometer (device_id, timestamp)
            """
        )
        return None

    try:
        _, conn, cur = run_with_retry(q_create, conn, cur, retries, retry_sleep)
        print("created index idx_acc_device_ts on accelerometer(device_id, timestamp)")
    except mysql.connector.Error as e:
        # Continue even if CREATE INDEX is not permitted; query logic still works.
        print(f"warning: could not create idx_acc_device_ts ({e})")
    return conn, cur


def main() -> None:
    parser = argparse.ArgumentParser(description="Build device/day availability map from accelerometer metadata only.")
    ...
    parser.add_argument("--sleep-per-day", type=float, default=0.2)
    ...
    overview_path = args.out_dir / "device_overview.csv"
    days_path = args.out_dir / "device_days.csv"
    ...
    overview_f, overview_w = open_csv_writer(overview_path, overview_fields)
    days_f, days_w = open_csv_writer(days_path, day_fields)

    conn, cur = reconnect()
    try:
        conn, cur = ensure_device_ts_index(conn, cur, args.retries, args.retry_sleep)
        for p_idx, (patient_label, device_ids) in enumerate(patients, start=1):
            ...
            for device_id in device_ids:
                try:
                    ...
                    first_local_ts = pd.to_datetime(first_ms, unit="ms", utc=True).tz_convert(args.tz)
                    last_local_ts = pd.to_datetime(last_ms, unit="ms", utc=True).tz_convert(args.tz)
                    first_day_local = first_local_ts.floor("D")
                    last_day_local = last_local_ts.floor("D")
                    approx_days_span = int((last_day_local - first_day_local).days) + 1
                    ...
                    overview_w.writerow(
                        {
                            ...
                            "last_ts_ms": last_ms,
                            "first_local": str(first_local_ts),
                            "last_local": str(last_local_ts),
                            "approx_days_span": approx_days_span,
                            "status": "ok",
                        }
                    )
                    overview_f.flush()

                    day_buckets, conn, cur = get_device_day_buckets(
                        device_id=device_id,
                    ...
                    for day_idx, (day_bucket, n_rows_day_opt) in enumerate(day_buckets, start=1):
                        ...
                        days_w.writerow(
                            {
                                ...
                                "n_rows_day": n_rows_day_opt if args.with_counts else "",
                            }
                        )
                        days_f.flush()
                        print(f"device={device_id} day_bucket={day_idx}/{total_days}")
