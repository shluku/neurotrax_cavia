...
def first_available_ts_after_t1(conn, device_id: str, t1_start_ms: int) -> int | None:
    cur = conn.cursor()
    try:
        cur.execute(
            """
...
def selected_adjusted_window_for_patient(conn, patient: pd.Series, device_ids: list[str]) -> dict[str, Any]:
    ...
    for device_id in device_ids:
        first_ts = first_available_ts_after_t1(conn, device_id, t1_start_ms)
        if first_ts is None:
        ...
        row = count_rows(conn, device_id, start_ms, end_ms)
...
def build_adjusted_readme(patient_count: int) -> str:
    return f"""# Phase 3 Significant Adjusted First-Available 7-Day Extraction
...
def main() -> None:
                ...
                for attempt in range(3):
                    try:
                        ...
                        if "start_ms" not in window:
                            table_status = "missing_no_adjusted_first_available_significant_rows_after_T1"
                        else:
                ...
                long_rows.append(
                    {
                        ...
                        "rows_in_window": len(rows),
                        "device_ids_used": ";".join(device_ids_used),
                        "calculation_context": "all_t1_patients_significant_adjusted_first_available_7d_after_T1",
                        "error_message": error_message,
                    }
                )
    finally:
    ...
    README_PATH.write_text(build_adjusted_readme(len(patients)), encoding="utf-8")
