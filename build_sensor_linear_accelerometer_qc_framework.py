...
TABLE_CONFIG = {
    "sensor_linear_accelerometer": {
        "has_column": "has_sensor_linear_accelerometer_metadata_after_T1",
        "description": "linear-acceleration sensor metadata",
        "raw_signal_table": "linear_accelerometer",
    },
    "sensor_accelerometer": {
        "has_column": "has_sensor_accelerometer_metadata_after_T1",
        "description": "general accelerometer sensor metadata",
        "raw_signal_table": "accelerometer",
    },
}
...
def output_paths(table_name: str) -> tuple[Path, Path, Path]:
    return (
        OUT_DIR / f"{table_name}_qc_by_patient.csv",
        OUT_DIR / f"{table_name}_qc_by_device_window.csv",
        OUT_DIR / f"README_{table_name}_qc.md",
    )


def fetch_rows(conn, table_name: str, device_id: str, start_ms: int, end_ms: int, limit: int) -> list[dict[str, Any]]:
    cur = conn.cursor(dictionary=True)
    quoted = safe_table_name(table_name)
    try:
        cur.execute(
            f"""
...
def build_table_readme(table_name: str, patient_count: int, rows_with_metadata: int, readiness_counts: pd.Series) -> str:
    config = TABLE_CONFIG[table_name]
    return f"""# Accelerometer Framework: {table_name} QC

This file documents the QC/readiness scan for `{table_name}`.

Order of work:

1. Use sensor metadata tables as QC/device-context layers.
2. Use QC results to decide whether and how to analyze `{config["raw_signal_table"]}`.
...
- For each mapped device, searched for the first `{table_name}` timestamp at or after T1.
...
- Did not extract raw movement features from `{config["raw_signal_table"]}`.

Why this metadata table matters:

- It describes Android {config["description"]}.
...
- `{config["raw_signal_table"]}` phone motion is not the same as body movement.

Current run:

- Patients checked: `{patient_count}`
- Patients with any post-T1 `{table_name}` metadata: `{rows_with_metadata}`

QC readiness distribution:

```text
{readiness_counts.to_string() if not readiness_counts.empty else "none"}
```

Generated files:

- `{table_name}_qc_by_patient.csv`
- `{table_name}_qc_by_device_window.csv`
"""


def build_framework_readme() -> str:
    return """# Accelerometer Framework
...
def run_table(table_name: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    patient_qc_path, device_qc_path, table_readme_path = output_paths(table_name)
    has_column = TABLE_CONFIG[table_name]["has_column"]
    ...
    try:
        for patient_index, (_, patient) in enumerate(patients.iterrows(), start=1):
            ...
            for device_id in device_ids:
                ...
                first_ts = first_available_ts_after_t1(conn, safe_table_name(table_name), device_id, t1_start_ms)
                if first_ts is None:
                    row = {
                        **base,
                ...
                coverage = count_rows(conn, safe_table_name(table_name), device_id, start_ms, end_ms)
                n_rows = int(coverage.get("n_rows") or 0)
                rows = fetch_rows(conn, table_name, device_id, start_ms, end_ms, MAX_FETCH_ROWS_PER_DEVICE_WINDOW) if n_rows else []
    ...
    readiness_counts = patient_df["qc_readiness_level"].value_counts(dropna=False) if not patient_df.empty else pd.Series(dtype=int)
    table_readme_path.write_text(
        build_table_readme(
            table_name=table_name,
            patient_count=len(patient_df),
            rows_with_metadata=int(patient_df[has_column].sum())
    ...
    README_PATH.write_text(build_framework_readme(), encoding="utf-8")

    print(f"table_name: {table_name}")
    print(f"patients_checked: {len(patient_df)}")
    print(
        f"patients_with_{table_name}_metadata_after_T1: "
        f"{int(patient_df[has_column].sum()) if not patient_df.empty else 0}"
    )
    ...
    for path in [patient_qc_path, device_qc_path, table_readme_path, README_PATH]:
        print(path)
    return patient_df, device_df
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="sensor_linear_accelerometer",
        help="Sensor metadata table to scan.",
    )
